...
void section_PMT (u_char *b, int len)
{
  ...
  while (len1 > 4) {
    ...
    // store stream type here for proper PES sub-decoding
    store_StreamTypeToMem(p2.elementary_PID, p2.stream_type);
    ...
    out_NL (3);
    out_S2B_NL (3,"Stream_type: ",p2.stream_type, dvbstrStream_TYPE (p2.stream_type));
