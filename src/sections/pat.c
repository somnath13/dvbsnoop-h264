...
void section_PAT (u_char *b, int len)
{
  ...
  for (; n>=4; n=n-4) {
      ...
      // som: remember this PMT PID
      if (pl.program_number) set_PMT_PID(pl.network_pmt_PID);
      ...
      out_SW_NL (3,(pl.program_number) ?"Program_map_PID: " :"Network_PID: ", pl.network_pmt_PID);
  }
  indent (-1);
  out_NL (3);
