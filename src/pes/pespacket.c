...
void decodePS_PES_packet (u_char *b, u_int len, int pid)
{
  ...
  PMT_stream_type = get_StreamTypeFromMem(pid);
  ...
  if (PMT_stream_type == 0x1B) {
    ...
    stream_id = outBit_S2x_NL(3,"H.264 NALU:", b, 27, 5, (char *(*)(u_long))dvbstrPESH264_NALU_ID );
  ...
  stream_id = outBit_S2x_NL(3,"Stream_id: ", b, 24, 8, (char *(*)(u_long))dvbstrPESstream_ID ); 
...
#if 0 // som: OBSOLETE code, but could be useful as a reference
